...
def Knapsack_TakeOrSkip(
    VALUES: list[int], WEIGHTS: list[int], capacity: int, item: int = 0
):
    ...
    if capacity >= WEIGHTS[item]:
        take = (
            Knapsack_TakeOrSkip(VALUES, WEIGHTS, capacity - WEIGHTS[item], item + 1)
            + VALUES[item]
        )
...
def Knapsack_FindNext(
    ...
    for choice in range(item, len(VALUES)):
        ...
        if WEIGHTS[choice] <= capacity:
            take = (
                Knapsack_FindNext(
                    VALUES, WEIGHTS, capacity - WEIGHTS[choice], choice + 1
                )
                + VALUES[choice]
            )
...
if __name__ == "__main__":
    ...
    if use_random_input:
        for _ in range(num_tests):
            ...
            ITEM_VALUES = randomIntArray(
                10, 500, length=num_items, allow_duplicates=False
            )
            ITEM_WEIGHTS = randomIntArray(
                10, 50, length=num_items, allow_duplicates=False
            )
            max_capacity = randint(min(ITEM_WEIGHTS), sum(ITEM_WEIGHTS))
            take_or_skip = Knapsack_TakeOrSkip(ITEM_VALUES, ITEM_WEIGHTS, max_capacity)
