...
def randomString(letters: str = string.ascii_lowercase, length: int = 5) -> str:
    return "".join(random.choice(letters) for _ in range(length))


def randomLowerCaseStrings(
...
def randomIntArray(
    low=-20, high=20, length=15, ordered=False, increasing=True, allow_duplicates=True
) -> list[int]:
    """
...
def randomLetterArray(
    length: int = 15, ordered=False, increasing=True, allow_duplicates=True
) -> list[str]:
    """
